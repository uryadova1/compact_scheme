...
def periodic_xx(vect, N):
    return np.concatenate([[vect[N - 2]], [vect[N - 1]], vect, [vect[1]], [vect[2]]])


def periodic_x(vect, N):
    return np.concatenate([[vect[N - 1]], vect, [vect[1]]])
...
def CWA(hv, qv, uv, hu, qu, uu, r, N):
    ...
    while flag:
        ...
        if norm(h_wk, h_wk_1).all() < eps and norm(q_wk,
                q_wk_1).all() < eps:  # - пока бредик, потому что это все таки разные случаи
            flag = False
            print(k)
        elif k > 1000:
            exit(0)
